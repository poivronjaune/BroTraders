...
def update_queue_entry(url: str, state: str) -> None:
    queue = load_queue()
    queue[url] = state
    write_queue(queue)
...
def _probe_and_build_queue(start_dt: date, end_dt: date) -> dict[str, str]:
...
# ---------------------------------------------------------------------------
def repo_exists(year: int, month: int) -> bool:
...
def save_ticker_data(ticker: str, new_rows: pd.DataFrame, output_dir:str = OUTPUT_DIR) -> int:
...
def main():
    ...
    if os.path.exists(QUEUE_FILE):
        ...
        if answer == "S":
            ...
            queue = _probe_and_build_queue(start_dt, end_dt)
        else:
    ...
    for url in pending:
        ...
        if df is not None:
            ...
            for ticker, rows in df.groupby("Ticker"):
                total = save_ticker_data(ticker, rows, output_dir)
                print_info("Saved", f"{ticker}  {len(rows)} new rows  ({total} total)", COLOR_GREEN)
                log.info("Saved %s: %d new rows (%d total)", ticker, len(rows), total)
            update_queue_entry(url, STATE_COMPLETED)
        else:
            update_queue_entry(url, STATE_ERROR)
